...
The HPI Person Identifier (CPN) is mandatory for all PractitionerRole searches. The HPI does not allow systems to search by FacID only to return all the practitioners at a location. The HPI FacID, HPI OrgID and role code can be used to filter the search results for a particular person. The active attribute should be used to filter out practitioner roles that have ended.
